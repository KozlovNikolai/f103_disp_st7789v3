...
void print_port_state(const char *name, uint8_t number)
{
	...
	// --- Определяем указатель на порт по букве ---
	switch (name[0])
	{
	case 'A':
	case 'a':
	...
	// --- Определяем режим пина ---
	uint32_t mode_bits;
	if (number < 8)
		mode_bits = (port->CRL >> (number * 4)) & 0xF; // для пинов 0–7
	else
		mode_bits = (port->CRH >> ((number - 8) * 4)) & 0xF; // для пинов 8–15

	uint8_t dir = (mode_bits & 0x3) ? 1 : 0; // MODE != 0 → выход

	// --- Считываем логическое состояние ---
	uint8_t val = (port->IDR & (1 << number)) ? 1 : 0;

	printf("P%s%d: %s, state=%d\r\n", name, number,
		   dir ? "Output" : "Input", val);
}
